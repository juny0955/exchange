...
import dev.junyoung.exchange.orderservice.application.port.in.HandleEngineCanceledEventUseCase;
...
public class EngineCanceledEventHandler implements HandleEngineCanceledEventUseCase {
	...
	public void handle(OrderId orderId, AccountId accountId) {
		...
		orderHistoryRepository.save(OrderHistory.createTransition(order, fromStatus, OrderHisReason.ENGINE_CANCELED));
	}
}
